...
import { KPIDataSource, KPIRecordEntry, KPIChannelEntry, KPITimeSeriesEntry, KPISentimentEntry, KPICategory } from '../types';
import {
  Plus, Pencil, Trash2, TrendingUp, TrendingDown, Download, Info, X,
  Filter, Target, ChartLine, Activity,
} from 'lucide-react';
import {
...
function hasRecords(data: unknown[]): boolean {
...
export default function KPIDashboard() {
  ...
  const activeKpis = useMemo(() => editableKpis.filter(k => k.isActive), [editableKpis]);
  const kpiMap = useMemo(() => new Map(activeKpis.map(k => [k.id, k])), [activeKpis]);
  ...
  const kpiRecords = useMemo(() => {
    const byKpi = selectedKpiId === 'all' ? kpiRecordData : kpiRecordData.filter(r => r.kpiId === selectedKpiId);
    const q = recordSearch.trim().toLowerCase();
    const filtered = q
      ? byKpi.filter(r => {
        const kpiName = kpiMap.get(r.kpiId)?.name.toLowerCase() || '';
        return kpiName.includes(q) || r.periodLabel.toLowerCase().includes(q) || r.unit.toLowerCase().includes(q);
      })
      : byKpi;

    return [...filtered].sort((a, b) => new Date(b.addedAt).getTime() - new Date(a.addedAt).getTime());
  }, [selectedKpiId, recordSearch, kpiRecordData, kpiMap]);

  const drilldownData = useMemo(() => {
    if (!drilldownKpiId) return null;
    const kpi = kpiMap.get(drilldownKpiId);
    if (!kpi) return null;
    ...
    return { kpi, records, latest, trend };
  }, [drilldownKpiId, kpiMap, kpiRecordData]);

  const overviewSummary = useMemo(() => {
    const tracked = cards.filter(c => c.records.length > 0).length;
    const improving = cards.filter(c => c.records.length > 1 && c.trend > 0).length;
    return { tracked, total: cards.length, improving, records: kpiRecordData.length };
  }, [cards, kpiRecordData.length]);

  useEffect(() => {
    if (activeTab === 'records' && !recordForm.kpiId && activeKpis[0]) {
      setRecordForm(prev => ({ ...prev, kpiId: activeKpis[0].id }));
    }
  }, [activeTab, activeKpis, recordForm.kpiId]);
  ...
  const saveRecord = () => {
    if (!recordForm.kpiId || !recordForm.periodLabel.trim() || !recordForm.value.trim() || !recordForm.unit.trim()) {
      setRecordError('KPI, Period, Value and Unit are required.');
      return;
    }

    const value = toNumber(recordForm.value);
    const target = recordForm.target.trim() ? toNumber(recordForm.target) : undefined;

    const payload = {
      kpiId: recordForm.kpiId,
      periodLabel: recordForm.periodLabel.trim(),
      value,
      target,
  ...
  const removeRecord = (id: string) => {
    if (window.confirm('Delete this KPI record? This cannot be undone.')) {
      deleteKpiRecord(id);
    }
  };
    ...
    <div className="p-6 max-w-7xl mx-auto">
      <div className="flex flex-wrap items-center justify-between gap-3 mb-6">
        <div>
          ...
          <p className="text-sm text-slate-400">Enterprise KPI reporting, drilldowns, trendlines and structured CRUD.</p>
        </div>
      ...
      <div className="bg-slate-900/70 border border-slate-800 rounded-xl p-3 mb-6 flex items-center gap-3 text-sm text-slate-300">
        ...
        KPIs are managed in <span className="font-semibold text-white">Admin → KPI Management</span>. Click KPI cards for deeper reporting.
      </div>

      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-6">
        <SummaryTile icon={<Target size={14} />} label="KPIs tracked" value={`${overviewSummary.tracked}/${overviewSummary.total}`} />
        <SummaryTile icon={<TrendingUp size={14} />} label="Improving KPIs" value={String(overviewSummary.improving)} />
        <SummaryTile icon={<Activity size={14} />} label="Total measurements" value={String(overviewSummary.records)} />
        <SummaryTile icon={<ChartLine size={14} />} label="Sources" value={String(new Set(kpiRecordData.map(r => r.source)).size)} />
      </div>
      ...
      <div className="flex gap-2 overflow-x-auto mb-6">
        {(['overview', 'records', 'channels', 'timeseries', 'sentiment'] as const).map(tab => (
          <button
            ...
            className={`px-4 py-2 rounded-lg text-sm capitalize transition-colors ${activeTab === tab ? 'bg-brand-600 text-white' : 'bg-slate-900 text-slate-400 border border-slate-800 hover:text-white'}`}
          >
            {tab}
          </button>
        ))}
        ...
        <div className="space-y-6">
          {!hasRecords(cards) && (
            <div className="rounded-xl border border-slate-800 bg-slate-900 p-4 text-sm text-slate-400">
              No KPI cards available. Create KPIs in Admin KPI management first.
            </div>
          )}
            ...
            {cards.map(card => (
              ...
              >
                ...
                <div className="text-xs mt-2 text-slate-400 flex items-center gap-1">
                  ...
                  {card.records.length > 1 ? `${card.trend >= 0 ? '+' : ''}${card.trend.toFixed(1)}% vs previous` : 'Click for detailed reporting'}
                </div>
              </button>
            ))}
          </div>
            ...
            <div className="bg-slate-900 border border-slate-800 rounded-xl p-5">
              <div className="flex items-center justify-between mb-4">
                <div>
                  ...
                  <p className="text-xs text-slate-400 mt-1">{drilldownData.records.length} entries • category: {CATEGORY_LABELS[drilldownData.kpi.category]}</p>
                </div>
            ...
            <div className="flex flex-wrap items-center justify-between gap-2 mb-3">
              ...
              <div className="flex items-center gap-2">
                <div className="relative">
                  ...
                  <input value={recordSearch} onChange={e => setRecordSearch(e.target.value)} placeholder="Search records" className="pl-7 pr-3 py-2 rounded-lg bg-slate-800 border border-slate-700 text-sm" />
                </div>
            ...
            <div className="overflow-auto">
              <table className="w-full text-sm">
                ...
                <tbody>
                  {kpiRecords.map(r => (
                    <tr key={r.id} className="border-b border-slate-800/70">
                      <td className="p-2">{kpiMap.get(r.kpiId)?.name || 'Unknown KPI'}</td>
                      <td className="p-2">{r.periodLabel}</td>
                      <td className="p-2">{r.value.toLocaleString()} {r.unit}</td>
                      <td className="p-2">{r.target ?? '--'}</td>
                      <td className="p-2">{SOURCE_BADGES[r.source]}</td>
                      <td className="p-2 text-right">
                        ...
                        <button disabled={!canEdit} onClick={() => removeRecord(r.id)} className="p-2 text-slate-400 hover:text-red-400 disabled:opacity-50"><Trash2 size={14} /></button>
                      </td>
                    </tr>
                  ))}
                  {kpiRecords.length === 0 && (
                    <tr>
                      <td className="p-4 text-slate-500 text-sm" colSpan={6}>No records match your filters.</td>
                    </tr>
                  )}
                </tbody>
              </table>
...
function ChannelSection({ data, onAdd, onEdit, onDelete, canEdit, sourceLabel, currentUser }: {
  ...
  const save = () => {
    ...
    const payload = {
      ...
      impressions: Math.max(0, toNumber(form.impressions)),
      clicks: Math.max(0, toNumber(form.clicks)),
      conversions: Math.max(0, toNumber(form.conversions)),
      spend: Math.max(0, toNumber(form.spend)),
  ...
  const remove = (id: string) => {
    if (window.confirm('Delete this channel row?')) onDelete(id);
  };

  return (
    <StructuredSection
      title={editingId ? 'Edit channel row' : 'Add channel row'}
      canEdit={canEdit}
      error={error}
      onCancel={editingId ? () => startEdit() : undefined}
      onSave={save}
      chart={
            ...
            {data.map(row => (
              <tr key={row.id} className="border-b border-slate-800/70">
                ...
                <td className="p-2 text-right">
                  ...
                  <button disabled={!canEdit} onClick={() => remove(row.id)} className="p-2 text-slate-400 hover:text-red-400 disabled:opacity-50"><Trash2 size={14} /></button>
                </td>
              </tr>
            ))}
          </tbody>
...
function TimeSeriesSection({ data, onAdd, onEdit, onDelete, canEdit, sourceLabel, currentUser }: {
  ...
  const save = () => {
    ...
    const payload = {
      ...
      impressions: Math.max(0, toNumber(form.impressions)),
      clicks: Math.max(0, toNumber(form.clicks)),
      leads: Math.max(0, toNumber(form.leads)),
      spend: Math.max(0, toNumber(form.spend)),
      applications: Math.max(0, toNumber(form.applications)),
      notes: form.notes.trim(),
      source: form.source,
      sourceLabel: form.source === 'manual' ? `Entered by ${currentUser}` : sourceLabel[form.source],
      addedBy: currentUser,
  ...
  const remove = (id: string) => {
    if (window.confirm('Delete this time-series row?')) onDelete(id);
  };

  return (
    <StructuredSection
      title={editingId ? 'Edit time-series row' : 'Add time-series row'}
      canEdit={canEdit}
      error={error}
      onCancel={editingId ? () => startEdit() : undefined}
      onSave={save}
...
function SentimentSection({ data, onAdd, onEdit, onDelete, canEdit, sourceLabel, currentUser }: {
  ...
  const save = () => {
    ...
    const payload = {
      date: form.date.trim(),
      positive: Math.max(0, toNumber(form.positive)),
      neutral: Math.max(0, toNumber(form.neutral)),
      negative: Math.max(0, toNumber(form.negative)),
      source: form.source,
      sourceLabel: form.source === 'manual' ? `Entered by ${currentUser}` : sourceLabel[form.source],
      addedBy: currentUser,
      addedAt: new Date().toISOString(),
  ...
  const remove = (id: string) => {
    if (window.confirm('Delete this sentiment row?')) onDelete(id);
  };

  return (
    <StructuredSection
      title={editingId ? 'Edit sentiment row' : 'Add sentiment row'}
      canEdit={canEdit}
      error={error}
      onCancel={editingId ? () => startEdit() : undefined}
      onSave={save}
  ...
  return (
    <div className="grid grid-cols-1 xl:grid-cols-3 gap-5">
      <div className="bg-slate-900 border border-slate-800 rounded-xl p-4 space-y-3">
        ...
        <div className="flex gap-2">
          <button disabled={!canEdit} onClick={onSave} className="flex-1 px-4 py-2 rounded-lg bg-brand-600 text-white text-sm disabled:opacity-50">Save</button>
          {onCancel && <button onClick={onCancel} className="px-4 py-2 rounded-lg bg-slate-800 border border-slate-700 text-sm">Cancel</button>}
        </div>
      </div>
      <div className="xl:col-span-2 bg-slate-900 border border-slate-800 rounded-xl p-4">
        {chart}
        <div className="overflow-auto mt-4">{table}</div>
      </div>
    </div>
  );
}
